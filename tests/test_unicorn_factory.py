...
from src.unicorn_factory import AutomatedBusinessPlan, MarketIntelligence, RapidScalingEngine, UnicornCompany, UnicornFactory
...
class TestAutomatedBusinessPlan:
    ...
    async def test_generate_plan(self):
        planner = AutomatedBusinessPlan()
        market = {"name": "AI Platform", "size": 100e9, "growth": 0.5, "competition": "low"}
...
class TestRapidScalingEngine:
    ...
    async def test_scale_company(self):
        engine = RapidScalingEngine()
        company = UnicornCompany(
            id="test-1", name="Scale Test", industry="AI", business_model="SaaS", arr=1e6, growth_rate=0.3
        )
