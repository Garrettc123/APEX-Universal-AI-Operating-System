// supabase/functions/atlas-ingest/index.ts
...
async function verifyStripe(body: string, sig: string): Promise<boolean> {
  try {
    ...
    const key = await crypto.subtle.importKey("raw", enc.encode(STRIPE_WEBHOOK_SECRET),
...
// Routes source:event_type → ATLAS task_types (comma-separated)
...
async function dispatchAtlas(webhookId: string, source: string, eventType: string, payload: unknown) {
  const tasks = (ROUTING[`${source}:${eventType}`] ?? "log").split(",");
  await fetch(`${ATLAS_API_URL}/api/v1/dispatch`, {
    method: "POST",
    headers: { "Content-Type": "application/json", "Authorization": `Bearer ${ATLAS_API_KEY}` },
    body: JSON.stringify({ webhook_event_id: webhookId, source, event_type: eventType, task_types: tasks, payload }),
  });
}
...
serve(async (req: Request) => {
  ...
  if (source === "stripe") {
    const valid = await verifyStripe(body, req.headers.get("stripe-signature") ?? "");
    if (!valid) return new Response("Invalid Stripe signature", { status: 401 });
  }

  let eventId = crypto.randomUUID();
  let eventType = "unknown";
  if (source === "stripe") { eventId = payload.id as string; eventType = payload.type as string; }
  else if (source === "github") { eventId = req.headers.get("x-github-delivery") ?? eventId; eventType = req.headers.get("x-github-event") ?? "push"; }
  else if (source === "docusign") { eventId = payload.envelopeId as string; eventType = payload.event as string; }
  else if (source === "apollo") { eventId = payload.id as string ?? eventId; eventType = "lead.created"; }

  const { data: existing } = await supabase.from("webhook_events").select("id,status").eq("event_id", eventId).maybeSingle();
  if (existing?.status === "processed") return new Response(JSON.stringify({ ok: true, duplicate: true }), { status: 200, headers: { "Content-Type": "application/json" } });

  const { data: inserted, error } = await supabase.from("webhook_events").upsert({ event_id: eventId, source, event_type: eventType, payload, status: "processing" }, { onConflict: "event_id" }).select("id").single();
  ...
  return new Response(JSON.stringify({ ok: true, id: inserted.id, source, event_type: eventType }),
