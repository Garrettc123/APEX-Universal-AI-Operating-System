...
-- Migration 0002 — complements 0001 (revenue_state/subscriptions/breakthroughs).
-- Idempotent: safe to re-apply (IF NOT EXISTS indexes, guarded triggers).
-- ============================================================
...
DROP TRIGGER IF EXISTS trg_webhook_updated ON webhook_events;
CREATE TRIGGER trg_webhook_updated  BEFORE UPDATE ON webhook_events  FOR EACH ROW EXECUTE FUNCTION update_updated_at();
DROP TRIGGER IF EXISTS trg_revenue_updated ON revenue_ledger;
...
DROP TRIGGER IF EXISTS trg_deploys_updated ON deployments;
...
DROP POLICY IF EXISTS service_all_webhook  ON webhook_events;
CREATE POLICY service_all_webhook  ON webhook_events     FOR ALL TO service_role USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS service_all_revenue  ON revenue_ledger;
