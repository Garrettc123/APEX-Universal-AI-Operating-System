...
For supported frameworks (Next.js, Sveltekit, Remix, Create React App, Nuxt, Vue, Astro, and more), using the package manager of your choice, add the `@vercel/analytics` package to your project:
...
For other frameworks (vanilla JavaScript, etc.), import the `inject` function from the package, which will add the tracking script to your app. **This should only be called once in your app, and must run in the client**.
